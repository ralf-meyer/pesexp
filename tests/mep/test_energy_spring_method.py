...
class SineSquaredCalculator(TwoDCalculator):
    ...
    def gradient(self, x, y):
        if 0.0 < x < 1.0:
            return np.pi * np.sin(2 * np.pi * x), 0.0
        return 0.0, 0.0


def test_energy_spring_method_1D():
    images = [
        Atoms(["H"], positions=[[x, 0.0, 0.0]])
        for x in [0.0, 0.125, 0.25, 0.375, 0.5, 0.625, 0.75, 0.875, 1.0]
    ]
    for im in images:
    ...
    np.testing.assert_allclose(np.diff(energies), [0.25] * 4 + [-0.25] * 4, atol=5e-3)
