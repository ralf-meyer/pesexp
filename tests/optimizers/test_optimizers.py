...
def test_max_step_internal(optimizer, maxstep=0.05, atol=1e-8):
    atoms = ase.atoms.Atoms(["H", "H"], positions=[[0.0, 0.0, 0.0], [1.5, 0.0, 0.0]])
    atoms.calc = ase.calculators.emt.EMT()
    coord_set = InternalCoordinates([Distance(0, 1)])
...
@pytest.mark.parametrize(
    "mol", ["H2O", "NH3", "CH4", "C2H4", "C2H6", "C6H6", "butadiene", "bicyclobutane"]
)
...
@pytest.mark.parametrize("coord_set", ["cart", "internal", "dlc", "anc"])
def test_optimizers_on_homoleptic_TMCs(
    resource_path_root, optimizer, ligand, coord_set
):
