...
# List of optimizers to test including their kwargs
optimizer_list = [
    (RFO, {"mu": 1}),
    (PRFO, {"mode": 0}),
    (PRFO, {"mode": 0, "mode_following": True}),
]


@pytest.mark.parametrize("optimizer,kwargs", optimizer_list)
def test_transition_state_cerjan_miller_surface(optimizer, kwargs, atol=1e-2):
    ...
    opt = optimizer(atoms, coordinate_set=coord_set, H0=H, maxstep=0.05, **kwargs)
...
@pytest.mark.parametrize("optimizer,kwargs", optimizer_list)
def test_transition_state_adams_surface(optimizer, kwargs, atol=1e-2):
...
@pytest.mark.parametrize("optimizer,kwargs", optimizer_list)
def test_transition_state_mueller_brown_surface(optimizer, kwargs, atol=1e-2):
...
def test_second_order_saddle_point(optimizer, kwargs, atol=1e-2):
    """This function tests the behavior of TS-optimizers near a second order
    transition state of the form: x**2 - y**2 + (z**4 - z**2)."""
...
@pytest.mark.parametrize("optimizer,kwargs", optimizer_list)
def test_transition_state_ammonia(optimizer, kwargs):
...
@pytest.mark.parametrize("optimizer,kwargs", optimizer_list)
def test_transition_state_ethane(optimizer, kwargs):
