...
class GPRCalculator(MLCalculator):
    def __init__(
        ...
        **kwargs,
        ...
        MLCalculator.__init__(self, restart=restart, label=label, atoms=atoms, **kwargs)
    ...
    def add_data(self, atoms):
        ...
        # else: check if the new atoms object has the same length as previous
            ...
            if not 3 * len(atoms) == self.n_dim:
                raise ValueError(
                    "New data does not have the same number of "
        ...
        self.x_train = np.append(self.x_train, self._transform_input(atoms), axis=0)
        ...
        if self.mean_model is None:
            ...
            E = atoms.get_potential_energy() - self.mean_model.get_potential_energy(
                atoms=atoms
            )
    ...
    def delete_data(self, indices=None):
        ...
        self.F_train = np.delete(
            self.F_train.reshape(-1, self.n_dim), indices, 0
        ).reshape(-1)
    ...
    def _update_intercept(self):
        try:
            ...
            if self.normalize_y == "mean":
                ...
                self.intercept = np.max(self.E_train) + float(self.normalize_y[4:])
    ...
    def fit(self):
        ...
        if self.opt_restarts > 0:
            ...
            for ii in range(self.opt_restarts):
                ...
                if self.verbose > 0:
                    print(
                        f"Starting hyperparameter optimization {ii+1}/"
                ...
                try:
                    ...
                    if self.verbose > 0:
                        print(
                            "Finished hyperparameter optimization after "
                    ...
                    print(
                        "Cholesky factorization failed for parameters:",
                        self.kernel.theta,
                    )
                    print(e)
        ...
        k_mat[self.n_samples :, self.n_samples :] += (
            np.eye(self.n_samples * self.n_dim) / self.C2
        )
        ...
        if self.verbose > 0:
            ...
            F = -y[self.n_samples :]
            print(
                "Fit finished. Final RMSE energy = %f, RMSE force = %f."
                % (
    ...
    def log_marginal_likelihood(self):
        ...
        try:
            ...
            print("excepted LinAlgError for theta: ", np.exp(self.kernel.theta))
        ...
        log_marginal_likelihood = (
            -0.5 * self._target_vector.dot(alpha)
        ...
        temp = (
            np.multiply.outer(alpha, alpha) - cho_solve((L, True), np.eye(L.shape[0]))
        )[:, :, np.newaxis]
        d_log_marginal_likelihood = 0.5 * np.einsum("ijl,ijk->kl", temp, k_grad)
    ...
    def log_predictive_probability(self):
        ...
        d_log_pred_prob = np.sum(
            (
                ...
                - 0.5
                * (1 + alpha**2 / K_inv.diagonal())[:, np.newaxis]
                * K_inv.dot(Z).diagonal().T
            )
            / K_inv.diagonal()[:, np.newaxis],
            axis=0,
        )
        return log_pred_prob, d_log_pred_prob
    ...
    def build_kernel_matrix(self, X_star=None, eval_gradient=False):
        ...
        if X_star is None:
            ...
            return self.kernel(
                self.x_train, X_star, dx=True, dy=True, eval_gradient=eval_gradient
            )
