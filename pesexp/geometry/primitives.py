...
class InverseDistance(Distance):
    ...
    def value(self, xyzs):
        return 1.0 / Distance.value(self, xyzs)
...
class LinearAngle(Primitive):
    ...
    def derivative(self, xyzs):
        ...
        if self.axis == 0:
            ...
            dt[3 * self.i : 3 * (self.i + 1)] = (
                np.dot(deji, u) + np.dot(eji, -du) + np.dot(ejk, -du)
            )
            ...
            dt[3 * self.k : 3 * (self.k + 1)] = (
                np.dot(dejk, u) + np.dot(eji, du) + np.dot(ejk, du)
            )
        else:
            ...
            # derivative w.r.t. atom i: drji/dri = 1, drik/dri = -1
            dt[3 * self.i : 3 * (self.i + 1)] = (
                np.dot(deji, w) + np.dot(eji, -dw) + np.dot(ejk, -dw)
            )
            # derivative w.r.t. atom j: drji/drj = -1, drjk/drj = -1
            ...
            # derivative w.r.t atom k: drik/drk = 1, drjk/drk = 1
            dt[3 * self.k : 3 * (self.k + 1)] = (
                np.dot(dejk, w) + np.dot(eji, dw) + np.dot(ejk, dw)
            )
        return dt
...
class Dihedral(Primitive):
    ...
    def value(self, xyzs):
        ...
        w = np.arctan2(np.dot(np.cross(b, a), g / norm_g), np.dot(a, b))
        return w
