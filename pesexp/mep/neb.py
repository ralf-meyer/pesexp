...
EnergyWeightedSpringConstant = namedtuple(
    'EnergyWeightedSpringConstant', 'kl ku eref')


class Spring:
    def __init__(self, atoms1, atoms2, energy1, energy2, emax, k):
        self.atoms1 = atoms1
        self.atoms2 = atoms2
        self.energy1 = energy1
        self.energy2 = energy2
        if isinstance(k, EnergyWeightedSpringConstant):
            ei = max(energy1, energy2)
            if ei > k.eref:
                alpha = (emax - ei) / (emax - k.eref)
                self.k = (1 - alpha) * k.ku + alpha * k.kl
            else:
                self.k = k.kl
        else:
            self.k = k
...
class BaseNEB:
    def __init__(self, images, k=0.1, climb=False, parallel=False,
        ...
        if isinstance(k, (float, int, EnergyWeightedSpringConstant)):
            k = [k] * (self.nimages - 1)
        self.k = list(k)
