...
class EnergySpringMethod(ase.neb.ImprovedTangentMethod):
    ...
    def add_image_force(
        ...
        spring_force_1 = (
            self.mu * abs(energies[i - 1] - energies[i]) + spring1.nt * spring1.k
        )
        spring_force_2 = (
            self.mu * abs(energies[i + 1] - energies[i]) + spring2.nt * spring2.k
        )
        ...
        if isinstance(self._k, EnergyWeightedSpringConstant):
            ...
            for i in range(self.nimages - 1):
                ...
                if ei > self._k.e_ref:
                    ...
                    k.append((1 - alpha) * self._k.k_upper + alpha * self._k.k_lower)
                else:
