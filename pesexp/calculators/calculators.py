        ...
        with open(f"{self.label}.out", "r") as fileobj:
            ...
            for line in lineiter:
                if "Total atoms:" in line:
                    ...
                    self.results["energy"] = float(line.split()[2]) * ase.units.Hartree
                    ...
                    for i in range(N_atoms):
                        gradient[i, :] = list(map(float, next(lineiter).split()))
...
class OpenbabelFF(Calculator):
    ...
    ob_units = {
        "kcal/mol": ase.units.kcal / ase.units.mol,
    ...
    def initialize(self, atoms):
        ...
        if not self.ff.Setup(obMol):
            ...
            raise RuntimeError(f"Could not setup force field {self.parameters.ff}")
...
class CerjanMillerSurface(TwoDCalculator):
    ...
    def gradient(self, x, y):
        exp = np.exp(-(x**2))
        return (
            2 * (self.a - self.b * y**2) * exp * x * (1 - x**2),
...
class AdamsSurface(TwoDCalculator):
    ...
    def gradient(self, x, y):
        dx1 = (
            -17 / 2 * (x**2 - 2) * y * np.exp(-(x**2 + y**2) / 4)
            - 6 * x**2
            + 16 * x
            - 6 * y
        )
        dx2 = (
            -17 / 2 * (y**2 - 2) * x * np.exp(-(x**2 + y**2) / 4)
            + 3 * y**2
            + 8 * y
            - 6 * x
        )
...
class MuellerBrownSurface(TwoDCalculator):
    ...
    def gradient(self, x, y):
        v = self._v(x, y)
        return (
            (2 * self.a * (x - self.x0) + self.b * (y - self.y0)) @ v,
...
class LEPSPotential(TwoDCalculator):
    ...
    def energy(self, r_AB, r_BC):
        ...
        Q_AB = (
            ...
            * self.d_AB
            * (
                1.5 * np.exp(-2 * self.alpha * (r_AB - self.r0))
        ...
        Q_BC = (
            ...
            * self.d_BC
            * (
                1.5 * np.exp(-2 * self.alpha * (r_BC - self.r0))
        ...
        Q_AC = (
            ...
            * self.d_AC
            * (
                1.5 * np.exp(-2 * self.alpha * (r_AC - self.r0))
        ...
        J_AB = (
            ...
            * self.d_AB
            * (
                np.exp(-2 * self.alpha * (r_AB - self.r0))
        ...
        J_BC = (
            ...
            * self.d_BC
            * (
                np.exp(-2 * self.alpha * (r_BC - self.r0))
        ...
        J_AC = (
            ...
            * self.d_AC
            * (
                np.exp(-2 * self.alpha * (r_AC - self.r0))
